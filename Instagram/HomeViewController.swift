...
//  Instagram
//
//  Created by 吉田なつみ on 2021/10/17.
//
import UIKit
import Firebase
...
class HomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
   ...
   // var password: String = ""
    ...
    @objc func handleComentButton(_ sender: UIButton, forEvent event: UIEvent) {
        ...
        //Alertの表示
        let alert = UIAlertController( title: "コメントを入力してください", message: nil, preferredStyle: .alert)
        let okButton = UIAlertAction(title: "OK", style: UIAlertAction.Style.default) { (action: UIAlertAction) in
            if (alert.textFields?.first) != nil {
                ...
                if (Auth.auth().currentUser?.uid) != nil {
                    let commentText = (Auth.auth().currentUser?.displayName)! + ":" + (alert.textFields?.first?.text)! 
                    var updateValue: FieldValue
                    ...
                    postRef2.updateData(["coment": updateValue])
                    print("DEBUG_PRINT: コメントボタンがタップされました。")
